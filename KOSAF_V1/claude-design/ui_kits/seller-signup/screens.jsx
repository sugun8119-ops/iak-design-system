...
function SignupForm({ mobile, onDone }) {
  ...
  const submit = () => { setTried(true); if (v.name && v.id && v.pw && v.pw === v.pw2 && v.type && v.biz && v.email) onDone(); };
  return (
    <div>
      ...
      <KS.FormField layout={L} label="아이디" required error={req('id')} htmlFor="f-id"><KS.Input id="f-id" width={mobile ? 230 : W} value={v.id} onChange={set('id')} state={req('id') ? 'error' : undefined} placeholder="영문 소문자, 숫자 중 6~20자 조합." /><KS.Button variant="secondary" size={45}>중복 확인</KS.Button></KS.FormField>
      <KS.FormField layout={L} label="비밀번호" required error={req('pw')} htmlFor="f-pw"><KS.Input id="f-pw" type="password" width={W} value={v.pw} onChange={set('pw')} state={req('pw') ? 'error' : undefined} placeholder="영문+숫자+특수문자 8~24자 조합." /></KS.FormField>
      <KS.FormField layout={L} label="비밀번호 확인" required error={tried && (!v.pw2 || v.pw !== v.pw2) ? (v.pw2 ? '비밀번호가 일치하지 않습니다.' : '필수항목을 입력해주세요.') : undefined} htmlFor="f-pw2"><KS.Input id="f-pw2" type="password" width={W} value={v.pw2} onChange={set('pw2')} state={tried && (!v.pw2 || v.pw !== v.pw2) ? 'error' : undefined} placeholder="비밀번호를 한 번 더 입력하세요." /></KS.FormField>
      ...
      <KS.FormField layout={L} label="이메일" required error={req('email')}><KS.Input width={mobile ? 150 : 200} value={v.email} onChange={set('email')} state={req('email') ? 'error' : undefined} placeholder="" aria-label="이메일 아이디" /><span aria-hidden="true">@</span><KS.Input width={mobile ? 150 : 200} placeholder="" aria-label="이메일 도메인" /><KS.Select size="form" width={mobile ? '100%' : 200} placeholder="직접입력" options={['직접입력', 'naver.com', 'gmail.com']} aria-label="도메인 선택" /></KS.FormField>
      <KS.FormField layout={L} label="사업자등록번호" required error={req('biz')}><KS.Input width={W} value={v.biz} onChange={set('biz')} state={req('biz') ? 'error' : undefined} placeholder="숫자 10자리" aria-label="사업자등록번호" /></KS.FormField>
      <KS.FormField layout={L} label="회사주소" required><div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: mobile ? '100%' : undefined }}><div style={{ display: 'flex', gap: 10 }}><KS.Input width={mobile ? 220 : W} placeholder="우편번호" aria-label="우편번호" /><KS.Button variant="secondary" size={45} style={{ flex: '0 0 auto' }}>우편번호 검색</KS.Button></div><KS.Input width={W} placeholder="기본주소" /><KS.Input width={W} placeholder="상세주소" /></div></KS.FormField>
