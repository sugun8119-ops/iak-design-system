...
test('packaged stdio MCP exposes real data and rejects invalid lookups',async()=>{
 ...
 const health=JSON.parse((await call('health_check')).content[0].text);assert.equal(health.components,10);assert.equal(health.icons,97);assert.equal(health.tokens,162);
