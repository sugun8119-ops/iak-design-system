...
test('packaged stdio MCP exposes real data and rejects invalid lookups',async()=>{
 ...
 const health=JSON.parse((await call('health_check')).content[0].text);assert.equal(health.components,13);assert.equal(health.icons,97);assert.equal(health.tokens,162);
 assert.equal(JSON.parse((await call('get_component',{name:'Button'})).content[0].text).name,'Button');
 for(const name of ['Dialog','Menu','Table'])assert.equal(JSON.parse((await call('get_component',{name})).content[0].text).name,name);
