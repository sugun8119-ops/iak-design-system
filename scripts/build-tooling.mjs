...
await build({entryPoints:[at('packages/ui/src/index.tsx')],outfile:path.join(out,'index.js'),bundle:true,platform:'neutral',format:'esm',external:['react','react/jsx-runtime'],jsx:'automatic',banner:{js:'"use client";'},logLevel:'warning'});
...
await fs.copyFile(at('src/library/guide.md'),path.join(mcpData,'guide.md'));
...
await build({entryPoints:[at('src/developer/playground.tsx')],outfile:path.join(developer,'app.js'),bundle:true,format:'esm',platform:'browser',jsx:'automatic',minify:true,define:{'process.env.NODE_ENV':'"production"'},legalComments:'linked',logLevel:'warning'});
