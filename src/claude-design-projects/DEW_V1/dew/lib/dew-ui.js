...
(function(){
const h=React.createElement,IC=()=>window.DEW_ICONS||{};
...
const Button=React.forwardRef(function Button({variant='primary',size='md',loading=false,disabled,children,className,type='button',...p},ref){return h('button',{...p,ref,type,disabled:disabled||loading,'aria-busy':loading||undefined,className:cx('dew-button',`dew-button--${variant}`,`dew-button--${size}`,className)},h('span',{style:loading?{opacity:0}:undefined},children),loading&&h('span',{className:'dew-spinner','aria-hidden':'true'}))});
function useField(id,description,error,describedBy){const uid=React.useId();const inputId=id||`dew-${uid}`;return {inputId,helpId:`${inputId}-help`,descriptionId:cx(describedBy,(!!error||!!description)&&`${inputId}-help`)||undefined}}
function FieldShell({label,inputId,helpId,description,error,required,children}){return h('div',{className:'dew-field'},h('label',{htmlFor:inputId},label,required&&h('span',{'aria-hidden':'true'},' *')),children,(error||description)&&h('p',{id:helpId,className:cx('dew-help',!!error&&'dew-error')},error||description))}
function fieldFactory(tag,extra){return React.forwardRef(function Field({label,description,error,id,className,required,'aria-describedby':db,children,...p},ref){const f=useField(id,description,error,db);const props={...p,id:f.inputId,ref,required,'aria-describedby':f.descriptionId,'aria-invalid':error?true:p['aria-invalid'],className:cx('dew-input',extra,className)};if(tag==='textarea')props.rows=p.rows||4;return h(FieldShell,{...f,label,description,error,required},h(tag,props,tag==='select'?children:undefined))})}
const TextField=fieldFactory('input'),Textarea=fieldFactory('textarea','dew-textarea'),Select=fieldFactory('select');
const Checkbox=React.forwardRef(function Checkbox({label,className,...p},ref){return h('label',{className:cx('dew-control',className)},h('input',{...p,type:'checkbox',ref}),h('span',null,label))});
const Switch=React.forwardRef(function Switch({label,className,...p},ref){return h('label',{className:cx('dew-control','dew-switch',className)},h('input',{...p,type:'checkbox',role:'switch',ref}),h('span',{className:'dew-switch-track','aria-hidden':'true'}),h('span',null,label))});
function Badge({tone='neutral',className,children,...p}){return h('span',{...p,className:cx('dew-badge',`dew-badge--${tone}`,className)},children)}
function Card({title,footer,children,className,...p}){const id=React.useId();return h('section',{...p,'aria-labelledby':title?id:p['aria-labelledby'],className:cx('dew-card',className)},title&&h('h2',{id},title),h('div',null,children),footer&&h('footer',null,footer))}
function Skeleton({width='100%',height=20,circle=false,style,className,...p}){return h('span',{...p,'aria-hidden':'true',className:cx('dew-skeleton',className),style:{width,height,borderRadius:circle?'50%':undefined,...style}})}
...
function Dialog({trigger,title,description,children,footer,open,defaultOpen,onOpenChange,initialFocusRef,closeOnOutside=false,closeLabel='닫기',size='md',inline=false}){
 ...
 const content=h(React.Fragment,null,h('div',{className:'dew-dialog-overlay',onClick:()=>{if(closeOnOutside)set(false)}}),
  h('div',{ref,role:'dialog','aria-modal':'true','aria-labelledby':tid,className:`dew-dialog dew-dialog--${size}`},
   h('div',{className:'dew-dialog-heading'},h('h2',{id:tid,className:'dew-dialog-title'},title),h('button',{type:'button',className:'dew-dialog-close','aria-label':closeLabel,onClick:()=>set(false)},h(Glyph,{name:'eva:close-fill'}))),
   description&&h('p',{className:'dew-dialog-description'},description),h('div',{className:'dew-dialog-body'},children),footer&&h('div',{className:'dew-dialog-footer'},footer)));
...
function Menu({trigger,label,items,align='end',open,onOpenChange,inline=false,highlightedId}){
 ...
 const list=h('div',{ref:listRef,style:inline?undefined:pos||{position:'fixed',visibility:'hidden'},role:'menu','aria-label':label,className:cx('dew-menu',inline&&'dew-menu--inline'),'data-align':align,onKeyDown:key},items.map(item=>h(React.Fragment,{key:item.id},item.separatorBefore&&h('div',{role:'separator',className:'dew-menu-separator'}),
  h('div',{role:'menuitem',tabIndex:-1,'data-id':item.id,className:'dew-menu-item','data-danger':item.danger||undefined,'data-disabled':item.disabled||undefined,'aria-disabled':item.disabled||undefined,'data-highlighted':hi===item.id&&!item.disabled?'':undefined,onMouseEnter:()=>!item.disabled&&setHi(item.id),onClick:()=>{if(item.disabled)return;item.onSelect&&item.onSelect();set(false)}},item.label))));
 if(inline)return list;
 return h('span',{className:'dew-menu-anchor',ref:anchor},React.cloneElement(trigger,{ref:trig,'aria-haspopup':'menu','aria-expanded':isOpen,onClick:()=>set(!isOpen),onKeyDown:e=>{if(e.key==='ArrowDown'){e.preventDefault();set(true)}}}),isOpen&&ReactDOM.createPortal(list,document.body));
}
...
function Pagination({page,pageCount,onPageChange,disabled=false,label='페이지 탐색',previewState}){
 ...
 return h('nav',{className:'dew-pagination','aria-label':label,'data-preview-state':previewState},
...
function Table({caption,columns,rows,rowKey,sort,defaultSort,onSortChange,loading=false,error,emptyMessage='표시할 항목이 없습니다.',loadingMessage='데이터를 불러오는 중입니다.',minWidth=560,pagination,virtualization}){
 ...
 const cells=[];let cursor=0;const spacer=(a,b)=>{if(b>a)cells.push(h('tr',{key:'sp'+a,'aria-hidden':'true',className:'dew-table-spacer'},h('td',{colSpan:Math.max(1,columns.length),style:{height:(b-a)*rowHeight}})))};
 for(let i=first;i<end;i++){const row=visibleRows[i];if(virtual)spacer(cursor,i);cells.push(h('tr',{key:'r'+rowKey(row),'aria-rowindex':virtual||pagination?startIndex+i+2:undefined},columns.map(c=>h('td',{key:c.id,style:{textAlign:c.align||'left'}},virtual?h('div',{className:'dew-table-virtual-cell',style:{height:rowHeight}},c.cell(row)):c.cell(row)))));cursor=i+1}
 ...
 return h('div',{className:'dew-table-root'},
  h('div',{ref:viewport,className:cx('dew-table-scroll',virtual&&'dew-table-virtual'),style:virtual?{maxHeight:height}:undefined,role:'region','aria-label':`${caption} · 스크롤`,tabIndex:0,onScroll:virtual?e=>setScrollTop(e.currentTarget.scrollTop):undefined},
   h('table',{className:'dew-table',style:{minWidth},'aria-busy':loading||undefined,'aria-rowcount':!state&&(virtual||pagination)?rows.length+1:undefined},
    h('caption',null,caption),
    h('thead',null,h('tr',null,columns.map(c=>h('th',{key:c.id,scope:'col',style:{textAlign:c.align||'left'},'aria-sort':c.sortValue&&activeSort&&activeSort.columnId===c.id?activeSort.direction:undefined},c.sortValue?h('button',{type:'button',disabled:loading||!!error,className:'dew-table-sort',onClick:()=>toggle(c.id)},c.header,h('span',{'aria-hidden':'true'},activeSort&&activeSort.columnId===c.id?(activeSort.direction==='ascending'?'↑':'↓'):'↕'),h('span',{className:'dew-sr-only'},' 정렬')):c.header)))),
    h('tbody',null,state?h('tr',null,h('td',{className:'dew-table-state',colSpan:Math.max(columns.length,1)},state)):cells))),
  pagination&&h('div',{className:'dew-table-paging'},h('span',null,range),h(Pagination,{page,pageCount,onPageChange:changePage,disabled:loading||!!error,label:`${caption} 페이지 탐색`})),
  h('p',{className:'dew-sr-only',role:'status'},state||(pagination?`${page} / ${pageCount} 페이지, ${range}`:`${rows.length}개 항목`)));
}
...
function ToastView({title,description,tone='info',action,onClose,previewState}){return h('li',{className:`dew-toast dew-toast--${tone}`,role:'status'},
 h('span',{className:'dew-toast-icon'},h(Glyph,{name:toneIcons[tone]})),
 h('div',{className:'dew-toast-content'},h('div',{className:'dew-toast-title'},h('span',{className:'dew-sr-only'},labels[tone]+': '),title),description&&h('div',{className:'dew-toast-description'},description),action&&h('button',{type:'button',className:'dew-toast-action','aria-label':action.altText,onClick:action.onClick},action.label)),
 h('button',{type:'button','aria-label':`${title} 알림 닫기`,className:'dew-toast-close','data-preview-state':previewState,onClick:onClose},h(Glyph,{name:'eva:close-fill',size:18})))}
...
function ToastProvider({children,duration=5000,label='알림',container}){
 ...
 const vp=h('ol',{className:'dew-toast-viewport','aria-label':`${label} (F8)`,tabIndex:-1},visible.map(e=>h(ToastView,{key:e.id,...e,onClose:()=>dismiss(e.id)})));
...
function AlertDialog({trigger,title,description,confirmLabel='확인',cancelLabel='취소',danger=false,onConfirm,errorMessage='작업을 완료하지 못했습니다. 다시 시도해 주세요.',inline=false,previewState}){
 ...
 const content=h(React.Fragment,null,h('div',{className:'dew-dialog-overlay'}),h('div',{ref,role:'alertdialog','aria-modal':'true','aria-labelledby':tid,'aria-describedby':did,'aria-busy':pending||undefined,className:'dew-dialog dew-dialog--sm'},
  h('h2',{id:tid,className:'dew-dialog-title'},title),h('p',{id:did,className:'dew-dialog-description'},description),
  error&&h('p',{role:'alert',className:'dew-error'},errorMessage),pending&&h('p',{role:'status',className:'dew-alert-status'},'처리 중입니다.'),
  h('div',{className:'dew-dialog-footer'},h(Button,{ref:cancel,variant:'secondary',disabled:pending,onClick:()=>{if(!busy.current){setError(false);setOpen(false)}}},cancelLabel),h(Button,{variant:danger?'danger':'primary',loading:pending,onClick:e=>{e.preventDefault();confirm()}},confirmLabel))));
...
window.DEW={Icon,Button,TextField,Textarea,Select,Checkbox,Switch,Badge,Card,Skeleton,Dialog,Menu,Table,Pagination,ToastProvider,useToast,AlertDialog,preview:{ToastView,Glyph}};
})();
