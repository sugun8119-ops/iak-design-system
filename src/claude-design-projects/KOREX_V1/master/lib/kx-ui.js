...
(function(){
const h=React.createElement,IC=()=>window.KX_ICONS||{};
const cx=(...a)=>a.filter(Boolean).join(' ');
const has=(o,k)=>Object.prototype.hasOwnProperty.call(o,k);
const FOCUSABLE='button:not([disabled]),[href],input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])';
function Glyph({name,size=20}){const ic=IC()[name];return ic?h('svg',{width:size,height:size,viewBox:'0 0 24 24',fill:'none',stroke:'currentColor',strokeWidth:1.75,strokeLinecap:'round',strokeLinejoin:'round','aria-hidden':'true',focusable:'false',dangerouslySetInnerHTML:{__html:ic.body}}):null}

/* index.tsx */
function Icon({name,size=20,label,...p}){const I=IC();const ic=has(I,name)?I[name]:undefined;if(!ic)return null;return h('svg',{...p,width:size,height:size,viewBox:`0 0 ${ic.width} ${ic.height}`,fill:'none',stroke:'currentColor',strokeWidth:1.75,strokeLinecap:'round',strokeLinejoin:'round',role:label?'img':undefined,'aria-label':label,'aria-hidden':label?undefined:true,focusable:'false',dangerouslySetInnerHTML:{__html:ic.body}})}
const Button=React.forwardRef(function Button({variant='primary',size='md',loading=false,selected,disabled,children,className,type='button',...p},ref){return h('button',{...p,ref,type,'aria-pressed':selected===undefined?p['aria-pressed']:!!selected,disabled:disabled||loading,'aria-busy':loading||undefined,className:cx('kx-button',`kx-button--${variant}`,`kx-button--${size}`,className)},h('span',{style:loading?{opacity:0}:undefined},children),loading&&h('span',{className:'kx-spinner','aria-hidden':'true'}))});
function useField(id,description,error,describedBy){const uid=React.useId();const inputId=id||`kx-${uid}`;return {inputId,helpId:`${inputId}-help`,descriptionId:cx(describedBy,(!!error||!!description)&&`${inputId}-help`)||undefined}}
function FieldShell({label,inputId,helpId,description,error,required,children}){return h('div',{className:'kx-field'},h('label',{htmlFor:inputId},label,required&&h('span',{'aria-hidden':'true'},' *')),children,(error||description)&&h('p',{id:helpId,className:cx('kx-help',!!error&&'kx-error')},error||description))}
function fieldFactory(tag,extra){return React.forwardRef(function Field({label,description,error,id,className,required,'aria-describedby':db,children,...p},ref){const f=useField(id,description,error,db);const props={...p,id:f.inputId,ref,required,'aria-describedby':f.descriptionId,'aria-invalid':error?true:p['aria-invalid'],className:cx('kx-input',extra,className)};if(tag==='textarea')props.rows=p.rows||4;return h(FieldShell,{...f,label,description,error,required},h(tag,props,tag==='select'?children:undefined))})}
const TextField=fieldFactory('input'),Textarea=fieldFactory('textarea','kx-textarea'),Select=fieldFactory('select');
const Checkbox=React.forwardRef(function Checkbox({label,className,...p},ref){return h('label',{className:cx('kx-control',className)},h('input',{...p,type:'checkbox',ref}),h('span',null,label))});
const Switch=React.forwardRef(function Switch({label,className,...p},ref){return h('label',{className:cx('kx-control','kx-switch',className)},h('input',{...p,type:'checkbox',role:'switch',ref}),h('span',{className:'kx-switch-track','aria-hidden':'true'}),h('span',null,label))});
function Badge({tone='neutral',className,children,...p}){return h('span',{...p,className:cx('kx-badge',`kx-badge--${tone}`,className)},children)}
function Card({title,footer,tone='default',children,className,...p}){const id=React.useId();return h('section',{...p,'aria-labelledby':title?id:p['aria-labelledby'],className:cx('kx-card',tone!=='default'&&`kx-card--${tone}`,className)},title&&h('h2',{id},title),h('div',null,children),footer&&h('footer',null,footer))}
function Skeleton({width='100%',height=20,circle=false,style,className,...p}){return h('span',{...p,'aria-hidden':'true',className:cx('kx-skeleton',className),style:{width,height,borderRadius:circle?'50%':undefined,...style}})}
...
function Dialog({trigger,title,description,children,footer,open,defaultOpen,onOpenChange,initialFocusRef,closeOnOutside=false,closeLabel='닫기',size='md',inline=false}){
 ...
 const content=h(React.Fragment,null,h('div',{className:'kx-dialog-overlay',onClick:()=>{if(closeOnOutside)set(false)}}),
  h('div',{ref,role:'dialog','aria-modal':'true','aria-labelledby':tid,className:`kx-dialog kx-dialog--${size}`},
   h('div',{className:'kx-dialog-heading'},h('h2',{id:tid,className:'kx-dialog-title'},title),h('button',{type:'button',className:'kx-dialog-close','aria-label':closeLabel,onClick:()=>set(false)},h(Glyph,{name:'kx:close'}))),
   description&&h('p',{className:'kx-dialog-description'},description),h('div',{className:'kx-dialog-body'},children),footer&&h('div',{className:'kx-dialog-footer'},footer)));
...
function Menu({trigger,label,items,align='end',open,onOpenChange,inline=false,highlightedId}){
 ...
 const list=h('div',{ref:listRef,style:inline?undefined:pos||{position:'fixed',visibility:'hidden'},role:'menu','aria-label':label,className:cx('kx-menu',inline&&'kx-menu--inline'),'data-align':align,onKeyDown:key},items.map(item=>h(React.Fragment,{key:item.id},item.separatorBefore&&h('div',{role:'separator',className:'kx-menu-separator'}),
  h('div',{role:'menuitem',tabIndex:-1,'data-id':item.id,className:'kx-menu-item','data-danger':item.danger||undefined,'data-disabled':item.disabled||undefined,'aria-disabled':item.disabled||undefined,'data-highlighted':hi===item.id&&!item.disabled?'':undefined,onMouseEnter:()=>!item.disabled&&setHi(item.id),onClick:()=>{if(item.disabled)return;item.onSelect&&item.onSelect();set(false)}},item.label))));
 if(inline)return list;
 return h('span',{className:'kx-menu-anchor',ref:anchor},React.cloneElement(trigger,{ref:trig,'aria-haspopup':'menu','aria-expanded':isOpen,onClick:()=>set(!isOpen),onKeyDown:e=>{if(e.key==='ArrowDown'){e.preventDefault();set(true)}}}),isOpen&&ReactDOM.createPortal(list,document.body));
}
...
function Pagination({page,pageCount,onPageChange,disabled=false,label='페이지 탐색',previewState}){
 ...
 const win=[];const lo=Math.max(1,Math.min(current-2,count-4)),hi=Math.min(count,lo+4);for(let i=lo;i<=hi;i++)win.push(i);
 return h('nav',{className:'kx-pagination','aria-label':label,'data-preview-state':previewState},
  h('button',{type:'button',disabled:disabled||current===1,onClick:()=>onPageChange(current-1),'aria-label':'이전 페이지'},h(Glyph,{name:'kx:chevron-left',size:16})),
  win.map(n=>h('button',{key:n,type:'button',disabled,'aria-current':n===current?'page':undefined,'aria-label':n+' 페이지',onClick:()=>onPageChange(n)},n)),
  h('button',{type:'button',disabled:disabled||current===count,onClick:()=>onPageChange(current+1),'aria-label':'다음 페이지'},h(Glyph,{name:'kx:chevron-right',size:16})));
}
...
function Table({caption,columns,rows,rowKey,sort,defaultSort,onSortChange,loading=false,error,emptyMessage='표시할 항목이 없습니다.',loadingMessage='데이터를 불러오는 중입니다.',minWidth=560,pagination,virtualization}){
 ...
 const cells=[];let cursor=0;const spacer=(a,b)=>{if(b>a)cells.push(h('tr',{key:'sp'+a,'aria-hidden':'true',className:'kx-table-spacer'},h('td',{colSpan:Math.max(1,columns.length),style:{height:(b-a)*rowHeight}})))};
 for(let i=first;i<end;i++){const row=visibleRows[i];if(virtual)spacer(cursor,i);cells.push(h('tr',{key:'r'+rowKey(row),'aria-rowindex':virtual||pagination?startIndex+i+2:undefined},columns.map(c=>h('td',{key:c.id,style:{textAlign:c.align||'left'}},virtual?h('div',{className:'kx-table-virtual-cell',style:{height:rowHeight}},c.cell(row)):c.cell(row)))));cursor=i+1}
 ...
 return h('div',{className:'kx-table-root'},
  h('div',{ref:viewport,className:cx('kx-table-scroll',virtual&&'kx-table-virtual'),style:virtual?{maxHeight:height}:undefined,role:'region','aria-label':`${caption} · 스크롤`,tabIndex:0,onScroll:virtual?e=>setScrollTop(e.currentTarget.scrollTop):undefined},
   h('table',{className:'kx-table',style:{minWidth},'aria-busy':loading||undefined,'aria-rowcount':!state&&(virtual||pagination)?rows.length+1:undefined},
    h('caption',null,caption),
    h('thead',null,h('tr',null,columns.map(c=>h('th',{key:c.id,scope:'col',style:{textAlign:c.align||'left'},'aria-sort':c.sortValue&&activeSort&&activeSort.columnId===c.id?activeSort.direction:undefined},c.sortValue?h('button',{type:'button',disabled:loading||!!error,className:'kx-table-sort',onClick:()=>toggle(c.id)},c.header,h('span',{'aria-hidden':'true'},activeSort&&activeSort.columnId===c.id?(activeSort.direction==='ascending'?'↑':'↓'):'↕'),h('span',{className:'kx-sr-only'},' 정렬')):c.header)))),
    h('tbody',null,state?h('tr',null,h('td',{className:'kx-table-state',colSpan:Math.max(columns.length,1)},state)):cells))),
  pagination&&h('div',{className:'kx-table-paging'},h('span',null,range),h(Pagination,{page,pageCount,onPageChange:changePage,disabled:loading||!!error,label:`${caption} 페이지 탐색`})),
  h('p',{className:'kx-sr-only',role:'status'},state||(pagination?`${page} / ${pageCount} 페이지, ${range}`:`${rows.length}개 항목`)));
}

/* toast.tsx */
const labels={success:'성공',warning:'주의',error:'오류',info:'안내'};
const toneIcons={success:'kx:check-circle',warning:'kx:alert-triangle',error:'kx:alert-circle',info:'kx:info'};
const durationValue=n=>n===0?Infinity:Number.isFinite(n)&&n>0?Math.max(1000,n):5000;
function ToastView({title,description,tone='info',action,onClose,previewState}){return h('li',{className:`kx-toast kx-toast--${tone}`,role:'status'},
 h('span',{className:'kx-toast-icon'},h(Glyph,{name:toneIcons[tone],size:18})),
 h('div',{className:'kx-toast-content'},h('div',{className:'kx-toast-title'},h('span',{className:'kx-sr-only'},labels[tone]+': '),title),description&&h('div',{className:'kx-toast-description'},description),action&&h('button',{type:'button',className:'kx-toast-action','aria-label':action.altText,onClick:action.onClick},action.label)),
 h('button',{type:'button','aria-label':`${title} 알림 닫기`,className:'kx-toast-close','data-preview-state':previewState,onClick:onClose},h(Glyph,{name:'kx:close',size:18})))}
...
function ToastProvider({children,duration=5000,label='알림',container}){
 ...
 const vp=h('ol',{className:'kx-toast-viewport','aria-label':`${label} (F8)`,tabIndex:-1},visible.map(e=>h(ToastView,{key:e.id,...e,onClose:()=>dismiss(e.id)})));
...
function AlertDialog({trigger,title,description,confirmLabel='확인',cancelLabel='취소',danger=false,onConfirm,errorMessage='작업을 완료하지 못했습니다. 다시 시도해 주세요.',inline=false,previewState}){
 ...
 const content=h(React.Fragment,null,h('div',{className:'kx-dialog-overlay'}),h('div',{ref,role:'alertdialog','aria-modal':'true','aria-labelledby':tid,'aria-describedby':did,'aria-busy':pending||undefined,className:'kx-dialog kx-dialog--sm'},
  h('h2',{id:tid,className:'kx-dialog-title'},title),h('p',{id:did,className:'kx-dialog-description'},description),
  error&&h('p',{role:'alert',className:'kx-error'},errorMessage),pending&&h('p',{role:'status',className:'kx-alert-status'},'처리 중입니다.'),
  h('div',{className:'kx-dialog-footer'},h(Button,{ref:cancel,variant:'secondary',disabled:pending,onClick:()=>{if(!busy.current){setError(false);setOpen(false)}}},cancelLabel),h(Button,{variant:danger?'danger':'primary',loading:pending,onClick:e=>{e.preventDefault();confirm()}},confirmLabel))));
...
window.KX={Icon,Button,TextField,Textarea,Select,Checkbox,Switch,Badge,Card,Skeleton,Dialog,Menu,Table,Pagination,ToastProvider,useToast,AlertDialog,preview:{ToastView,Glyph}};
})();
