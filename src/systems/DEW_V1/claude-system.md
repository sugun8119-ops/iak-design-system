...
`preview/editorial.html`과 `preview/source/editorial-patterns.js`를 함께 사용한다. 추가 네임스페이스는 `window.DEWEditorial`이며 기존 DEW 컴포넌트를 덮어쓰지 않는다.

1. Kinfolk에서 참고한 기사 위계: 대표 기사 1개와 보조 기사 2개를 구분한다. 데스크톱은 2:1 영역, 모바일은 대표 기사 뒤에 보조 기사를 배치한다.
2. Gentlewoman에서 참고한 상세 편집: 본문 최대 680px, 넓은 그림·캡션, 인용문 최대 860px을 교차 배치한다. 본문 순서는 모바일에서도 유지한다.
3. It’s Nice That에서 참고한 후속 읽기: 관련 글 3개, 주제별 목록 링크, 구독 폼 순서로 배치한다.

원본 사진·문구·로고·폰트를 복제하지 않는다. DEW 기존 foundation을 유지하고 레퍼런스는 구성 원리만 참고한다. 새 패턴을 IAK 또는 Behance 원본 관찰 사례로 합산하지 않는다. 구독은 외부 전송 없는 시연이며 실제 신청 완료라고 표시하지 않는다.
